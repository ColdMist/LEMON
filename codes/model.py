...
class KGEModel(nn.Module):
    def __init__(self, model_name, nentity, nrelation, hidden_dim, gamma,
        ...
        if model_name not in ['TransE', 'DistMult', 'ComplEx', 'RotatE', 'pRotatE', 'TransD', 'QuatE']:
    ...
    def QuatE(self, head, relation, tail, head_t, tail_t, relation_t, mode):
        h, hi, hj, hk = torch.chunk(head, 4, dim=2)
        t, ti, tj, tk = torch.chunk(tail, 4, dim=2)

        r, ri, rj, rk = self.normalize_quaternion(relation)
    ...
    def test_step(model, test_triples, all_true_triples, args):
            ...
            for test_dataset in test_dataset_list:
                for positive_sample, negative_sample, filter_bias, mode in test_dataset:
                    ...
                    for i in range(batch_size):
                        ...
                        if args.collect_low_scores:
                            if ranking>args.low_score_collection_threshold:
                                low_score_triple = positive_sample[i,:]
                                low_score_triples.append(low_score_triple.cpu().detach().numpy())
        ...
        return metrics,low_score_triples
