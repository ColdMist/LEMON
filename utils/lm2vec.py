...
def process_embedding(datadir,emb_type="st", save_extra=None):
    save_dir = datadir + '/trained_embedding_LM'
    #print(datadir)
    #exit()
    ...
    try:
        ...
        pass
    np.save(os.path.join(save_dir, 'entity_embedding_'+ emb_type + '.npy'), entity_embedding_arr.squeeze(1))
    try:
        np.save(os.path.join(save_dir, 'relation_embedding_'+ emb_type + '.npy'), relation_embedding_arr.squeeze(1))
    except:
        pass
...
if __name__ == "__main__":
    ...
    target_type = "st"
    process_embedding("../data/wn18",emb_type=target_type)
    process_embedding("../data/wn18", emb_type="ft", save_extra=True)
    process_embedding("../data/wn18", emb_type="st", save_extra=True)
